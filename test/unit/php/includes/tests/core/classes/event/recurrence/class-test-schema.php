...
class Test_Schema extends Base {
	...
	/**
	 * Coverage for maybe_refresh_has_recurring_events ignoring unrelated meta keys.
	 *
	 * @covers ::maybe_refresh_has_recurring_events
	 *
	 * @return void
	 */
	...
	/**
	 * Coverage for added_post_meta on the canonical key triggering a refresh.
	 *
	 * @covers ::maybe_refresh_has_recurring_events
	 *
	 * @return void
	 */
	public function test_added_post_meta_on_canonical_key_refreshes_flag(): void {
		$post_id = $this->factory->post->create();

		// The frequency mirror is written first, simulating projection having
		// already run, so the canonical-key write below observes it.
		add_post_meta( $post_id, self::FREQUENCY_META_KEY, 'weekly' );
		update_option( Query::HAS_RECURRING_OPTION, '0', true );
		...
		$this->assertSame(
			'1',
			get_option( Query::HAS_RECURRING_OPTION ),
			'Failed to assert that adding the canonical recurrence meta key refreshes the flag.'
		);
	}

	/**
	 * Coverage for updated_post_meta on the canonical key triggering a refresh.
	 *
	 * @covers ::maybe_refresh_has_recurring_events
	 *
	 * @return void
	 */
	public function test_updated_post_meta_on_canonical_key_refreshes_flag(): void {
		$post_id = $this->factory->post->create();
		...
		$this->assertSame(
			'1',
			get_option( Query::HAS_RECURRING_OPTION ),
			'Failed to assert that updating the canonical recurrence meta key refreshes the flag.'
		);
	}
	...
	/**
	 * Coverage for trashing a post refreshing the flag via transition_post_status.
	 *
	 * @covers ::refresh_has_recurring_events
	 *
	 * @return void
	...
	/**
	 * Coverage for untrashing a post refreshing the flag via transition_post_status.
	 *
	 * @covers ::refresh_has_recurring_events
	 *
	 * @return void
	...
	/**
	 * Coverage for a hard delete refreshing the flag via deleted_post.
	 *
	 * @covers ::refresh_has_recurring_events
	 *
	 * @return void
	...
	public function test_setup_hooks(): void {
		...
		$hooks = array(
			...
			array(
				...
				'priority' => 10,
				'callback' => array( $instance, 'maybe_refresh_has_recurring_events' ),
			),
			array(
				'type'     => 'action',
